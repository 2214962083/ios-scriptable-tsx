...
class Widget extends Base {
  constructor(arg = '') {
    ...
    if (this.settings && this.settings['arg']) {
      this.arg = this.settings['arg'] as string
    }
  ...
  async render() {
    ...
    const data: any = await this._getData()
   ...
   * 句子类型设置
   */
  async actionSetting1(): Promise<void> {
  ...
  // 用户点击组件，触发的 action
  async actionMenus(content: string) {
